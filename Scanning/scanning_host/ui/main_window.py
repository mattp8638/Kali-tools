...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.setWindowTitle("Scanning App Host")
    ...
    def _create_header(self) -> QWidget:
        ...
        # App name
        title = QLabel("Scanning App Host")
        title.setStyleSheet("""
            font-size: 20px;
            font-weight: bold;
            color: #00d4ff;
    ...
    def _on_tool_finished(self, result: ScanResult):
        ...
        self._output.append_line("")
        self._output.append_line(f"[*] {result.tool_name} finished with status: {result.status.value}")
        if result.structured_data:
            data = result.structured_data
            if "hit_count" in data:
                self._output.append_line(f"[+] Hit count: {data['hit_count']}")
            if "count" in data:
                self._output.append_line(f"[+] Count: {data['count']}")
            if "finding_count" in data:
                self._output.append_line(f"[+] Findings: {data['finding_count']}")
            if "findings_count" in data:
                self._output.append_line(f"[+] Findings: {data['findings_count']}")
            if "hop_count" in data:
                self._output.append_line(f"[+] Hops: {data['hop_count']}")
            if "open_ports" in data:
                self._output.append_line(f"[+] Open ports: {len(data.get('open_ports', []))}")
            if "error" in data:
                self._output.append_line(f"[!] {data['error']}")
        self._output.append_line("[*] See Structured Results tab for full details.")
    ...
    def _show_about(self):
        ...
        QMessageBox.about(
            self,
            "About Scanning App Host",
            "<h2>Scanning App Host</h2>"
            "<p>Version 0.1.0</p>"
            "<p>A Windows desktop application for hosting and running "
            "Python scanning tools.</p>"
